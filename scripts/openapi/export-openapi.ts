...
async function run() {
    ...
    try {
        ...
        if (!['localhost', '127.0.0.1'].includes(parsed.hostname)) {
            throw new Error('Host must be localhost or 127.0.0.1');
        }
    } catch (err) {
        console.error('Invalid OPENAPI_SOURCE_URL:', err.message);
        process.exit(1);
    }
    ...
    if (basic) {
        const token = Buffer.from(`${basic.user}:${basic.pass}`).toString(
            'base64',
        );
